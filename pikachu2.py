...
class Pikachu2:
    ...
    def __init__(self):
        ...
        if Pikachu2.image == None:
            Pikachu2.image = load_image('Pikachu2.png')
        if Pikachu2.jump_sound == None:
            Pikachu2.jump_sound = load_wav('Data_3.wav')
            Pikachu2.jump_sound.set_volume(32)
        if Pikachu2.skill_sound == None:
            Pikachu2.skill_sound = load_wav('Data_2.wav')
            Pikachu2.skill_sound.set_volume(32)
        if Pikachu2.shadow_image == None:
            Pikachu2.shadow_image = load_image('shadow.png')
    ...
    def update(self, frame_time):
        ...
        distance = Pikachu2.RUN_SPEED_PPS * frame_time
        self.total_frames += Pikachu2.FRAMES_PER_ACTION * Pikachu2.ACTION_PER_TIME * frame_time
        self.frame = int(self.total_frames) % 5
        self.x += (self.dir * distance)
        self.x = clamp(250, self.x, 425)
    ...
    def get_bb(self):
        return self.x -15 , self.y +28 , self.x -5 , self.y +28
    ...
    def handle_event(self, event):
        ...
        #피카츄가 기술을 썼을때
            ...
            if self.state in (self.STAND, self.JUMP):
                #self.state = self.STAND
                self.dir = 0
        elif (event.type, event.key) == (SDL_KEYUP, SDLK_g):
            if self.state in (self.STAND, self.JUMP):
                #self.state = self.STAND
                self.dir = 0
